...
# stand in calculation of regional scaling rates from individual site rates #
# importing scaling rates
qq <- 0.05 #20-year storm
storm.duration <- 1/qq
lst.trend.obs.scaling.DAILY.HOURLY.rates.2m <- readRDS(file=paste0("./Data/model.trend.scaling/lst.GPD.at.site.trend.obs.scaling.DAILY.HOURLY.rates_",
                                                                   storm.duration,".yr.RDS"))
# [1)all wet days, 2)ARs, 3)non-ARs] [max precip, total precip] [TEMP, DEWP (concurrent,prior)]
lst.trend.obs.scaling.DAILY.HOURLY.rates.850hpa <- readRDS(file=paste0("./Data/model.trend.scaling/lst.GPD.at.site.trend.Combo.850.obs.scaling.DAILY.HOURLY.rates_",
                                                                       storm.duration,".yr.RDS"))
# [1)all wet days, 2)ARs, 3)non-ARs] [max precip, total precip] [TEMP, DEWP (concurrent,prior)]
bs.all.ensm.DY.2m <- lst.trend.obs.scaling.DAILY.HOURLY.rates.2m[[1]]
bs.all.ensm.HY.2m <- lst.trend.obs.scaling.DAILY.HOURLY.rates.2m[[2]]
bs.all.ensm.DY.850hpa <- lst.trend.obs.scaling.DAILY.HOURLY.rates.850hpa[[1]]
bs.all.ensm.HY.850hpa <- lst.trend.obs.scaling.DAILY.HOURLY.rates.850hpa[[2]]

nsites <- 112
num.samples <- 100000

ar.non.ars.scen.opts <- c('ARs','non-ARs')
ta_td_selec.opts <- c("Ta","Td","preTa","preTd")
num.t.options <- length(ta_td_selec.opts)
precip_selec.opts <- c("Maximum Precipitation","Total Precipitation")
num.precips <- length(precip_selec.opts)

region.all.my.at.site.files <- list()
all.my.at.site.files <- list(bs.all.ensm.DY.2m,bs.all.ensm.HY.2m,bs.all.ensm.DY.850hpa,bs.all.ensm.HY.850hpa)
for (k in 1:4){
  dummy <- array(NA,c(3,num.samples,num.precips,num.t.options))
  my.at.site.file <- all.my.at.site.files[[k]]
  bs.all.ensm.DY.2m 
  for (in.ar in 1:3){
    for (pr.in in 1:num.precips){
      for (temp.in in 1:num.t.options){
        set.seed(num.samples)
        my.rates <- my.at.site.file[in.ar,,pr.in,temp.in]
        my.mu <- mean(my.rates,na.rm=T)
        my.var <- var(my.rates,na.rm=T)
        my.sigma.sq = my.var/nsites
        my.sigma <- sqrt(my.sigma.sq)
        dummy[in.ar,,pr.in,temp.in] <- rnorm(num.samples,my.mu,my.sigma)
      }
    }
  }
  region.all.my.at.site.files[[k]] <- dummy
}

bs.all.ensm.DY.2m <- region.all.my.at.site.files[[1]]
bs.all.ensm.HY.2m <- region.all.my.at.site.files[[2]]
bs.all.ensm.DY.850hpa <- region.all.my.at.site.files[[3]]
bs.all.ensm.HY.850hpa <- region.all.my.at.site.files[[4]]
